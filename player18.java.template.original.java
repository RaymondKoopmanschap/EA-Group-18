    ...
    public void run() {
        ...
        while (evals < evaluations_limit_) {
            ...
            for (int i = 0; i <  0.1 * POPULATION_SIZE; i++) {
                //one-point crossover
                // from 1 to 9
                population.addAll(OnePointCrossover(parents));
                //population.addAll(WholeArithmeticRecombination(parents));
            }

            for (int i = 0; i <  10; i++) { // creates 10
                population.addAll(ArithmeticCrossover(parents, ARITHMETIC_XOVER_N_PARENTS));
            }
            ...
            boolean age_based_selection = false;
            if (N_SURVIVORS > population.size()) {
                population = population.subList(0, population.size());
            } else {
                population = population.subList(0, N_SURVIVORS);
            }
        }
            ...
            for (int i = 0; i < ISLANDS; i++) {
                ...
                */
                ...
                for (int k = 0; k < survivors.size(); k ++) {
                    ...
                    if (dice_roll < mutation_probability) {
                        //children.get(j).uniformMutation();
                        //survivors.get(k).nonUniformMutation();
                        survivors.get(k).UncorrelatedMutationOneStepSize();
                        //survivors.get(k).UncorrelatedMutationNStepSizes();
                    }
                }
                ...
                if (islands.get(i).generations_without_fitness_change > 30) {
                ...
                /*
                if (islands.get(i).generations_without_fitness_change > 20 && i % 3 == 0) {
                    //System.out.println("initialized new island");
                    //IslandMigration(islands.get(i), islands);
                    islands.set(i, InitializeRandomIsland(POPULATION_SIZE));
                }
